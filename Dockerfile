...
WORKDIR /app
COPY pom.xml .
RUN --mount=type=cache,id=cm-vertx-cache,target=/root/.m2 mvn clean package -Dmaven.test.skip  && rm -r target

COPY src ./src
RUN --mount=type=cache,id=cm-vertx-cache,target=/root/.m2 mvn package -Dmaven.test.skip
