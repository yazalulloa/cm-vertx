...
import com.yaz.cm.vertx.sse.SSEHeaders;
...
public class VertxHandlerImpl implements VertxHandler {
  ...
  @Override
  public void publishSse(String address) {

    final var deliveryOptions = new DeliveryOptions()
        .addHeader(SSEHeaders.EVENT, address);

    vertx.eventBus().publisher("/" + address, deliveryOptions)
        .write(null);
  }

    /*public Completable pagingFileBuffer(String path, int readBufferSize, int pageSize, OpenOptions openOptions, Function<Collection<Buffer>, Completable> function) {
        return Single.fromCallable(() -> vertx.fileSystem().open(path, openOptions))
                .flatMap(this::single)
                .map(asyncFile -> asyncFile.setReadBufferSize(readBufferSize))
                .map(BehaviorProcessor::createDefault)
                .flatMapCompletable(behaviorProcessor -> {


                    behaviorProcessor.concatMap(asyncFile -> {


                        return Flowable.<Buffer>create(emitter -> {

                                    asyncFile.handler(emitter::onNext)
                                            .endHandler(v -> {
                                                asyncFile.close();
                                                emitter.onComplete();
                                            })
                                ...
                                .toList(pageSize)
                                .flatMapCompletable(function::apply)
                                ;
